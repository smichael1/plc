...
/**
 * The class PlcTag describes an Allen-Bradley PLC tag.
 * <p>
...
public class PlcTag implements IPlcTag {
    ...
    public PlcTag(String tagName, int direction, String pcFormat, int plcioTimeoutMs,
                  int totalMembers, int totalByteLength, TagItem[] tagItems) throws Exception {
        ...
        constructTagMemberItemMaps(tagItems);
    ...
    private void constructTagMemberItemMaps(TagItem[] tagItems) throws ABPlcioExceptionBadPlcTagProperties {

        // create HashMap to store by item name a TagItem object for each item in tag
        tagItemMap = new HashMap<String,TagItem>(tagItems.length);
        // create HashMap storing by tag member number an ArrayList containing all
        // tag data item names contained in the member
        ...
        for (int pcFormatTypeDescNum = 0; pcFormatTypeDescNum < pcFormatTypeAL.size(); pcFormatTypeDescNum++) {
            ...
            while (bytesUsed < typeTotalBytes) {
                ...
                thisTagItem = tagItems[itemNum];

                String itemName = thisTagItem.getItemName();

                String itemTypeString = thisTagItem.getItemTypeString();

                switch (plcioTypeId) {
                    case PlcioPcFormat.TYPE_C: // char = 1 byte
                        if (itemTypeString.equals(PropTypes.BOOLEAN.getTypeString())) {
                            // up to 16 boolean tag items from bit 0 to bit 15 can be stored in each 16 bit short
                            int bitsUsed = 0;
                            while (itemTypeString.equals(PropTypes.BOOLEAN.getTypeString()) && (bitsUsed <= 15)) {
                                ...
                                if (bitsUsed > 15) {
                                    throw new ABPlcioExceptionBadPlcTagProperties("Too many boolean tag items " +
                                            ...
                                            "' created item: '" + itemTypeString + "'");
                                }

                                itemNum++;
                                if (itemNum >= tagItems.length) {
                                ...
                                if (itemTypeString.equals(PropTypes.BOOLEAN.getTypeString())) {
                                    int nextItemBitPos = thisTagItem.getBitPos();
                        ...
                        } else if (itemTypeString.equals(PropTypes.STRING.getTypeString())) {
                            ...
                            throw new ABPlcioExceptionBadPlcTagProperties("PLCIO pcFormat of type '" +
                                    ...
                                    itemName + "' can not be used to describe types stored in propertyDB as '" +
                                    itemTypeString + "'");
                        }
                    ...
                    case PlcioPcFormat.TYPE_I: // short = 2 bytes
                        ...
                        } else if (itemTypeString.equals(PropTypes.INTEGER.getTypeString())) {
                    ...
                    case PlcioPcFormat.TYPE_J: // integer = 4 bytes
                        if (itemTypeString.equals(PropTypes.BOOLEAN.getTypeString())) {
                            ...
                            int bitsUsed = 0;
                            while (itemTypeString.equals(PropTypes.BOOLEAN.getTypeString()) && (bitsUsed <= 31)) {
                    ...
                    case PlcioPcFormat.TYPE_Q: // long = 8 bytes
                        if (itemTypeString.equals(PropTypes.LONG.getTypeString())) {
                ...
                case PlcioPcFormat.TYPE_R: // real = 4 bytes
                        if (itemTypeString.equals(PropTypes.REAL.getTypeString())) {
                            tagItemMap.put(itemName, thisTagItem);
                            thisMemberItemNamesAL.add(itemName);
                            Log.debug(LOG_CAT_ITEM, 4, "Created: " + thisTagItem.toString());
                    ...
                    case PlcioPcFormat.TYPE_D: // double = 8 bytes
                        if (itemTypeString.equals(PropTypes.DOUBLE.getTypeString())) {
                            tagItemMap.put(itemName, thisTagItem);
                            thisMemberItemNamesAL.add(itemName);
                            Log.debug(LOG_CAT_ITEM, 4, "Created: " + thisTagItem.toString());
    ...
    public String getTagItemName(String itemName) {
        return tagItemMap.get(itemName).getItemName();
    }
    
    // Documented in IPlcTag
    @Override
    public String getTagItemTypeString(String itemName) {
        return tagItemMap.get(itemName).getItemTypeString();
    }
